...
### Common question: can we use managed identity via this Kerberos option instead of the storage key?

**No — and it's worth being precise about why, because "Kerberos" and "managed identity" are two unrelated mechanisms that are easy to conflate.**

- **Managed identity** is an Entra ID (Azure AD) concept: the Function App gets a service principal and requests OAuth2 bearer tokens to call Azure **REST APIs** (Blob/Queue/Table REST, ARM, Key Vault). It's HTTPS-based and has nothing to do with the SMB protocol.
- **Kerberos**, in the Azure Files SMB security settings, is SMB-protocol-level authentication: an identity source (AD DS, Microsoft Entra Domain Services, or Microsoft Entra Kerberos) issues a Kerberos *ticket* that the **connecting client's own OS** presents during SMB session setup. Notice the Security blade only ever exposes **NTLM v2** and **Kerberos** as authentication mechanisms — there is no "managed identity" checkbox, because Azure Files' SMB layer doesn't consume OAuth tokens at all; it only speaks NTLMv2 or Kerberos tickets.

All three ways to obtain an SMB Kerberos ticket for Azure Files require the *connecting client* to be domain-joined (AD DS) or Microsoft Entra-(hybrid-)joined (Microsoft Entra Domain Services / Microsoft Entra Kerberos), so its OS can request a ticket from that identity source. Azure Functions' Consumption/Premium compute is ephemeral, multi-tenant, platform-managed sandboxes — there is no domain join and no device identity for it to authenticate as. So even if Microsoft Entra Kerberos were enabled on the storage account, the Functions runtime's automatic content-share mount has no code path that would ever use it — it unconditionally uses the storage-account-key connection string, regardless of what identity options the account offers.

**Where managed identity genuinely does work today:**
- **`AzureWebJobsStorage`** (the runtime's own blob/queue/table storage for bindings, keys, and Durable Functions) *does* support an identity-based connection instead of a connection string — real managed identity, over REST. It does not touch the content share.
- **Flex Consumption plan** deploys the code package from Blob Storage rather than an SMB-mounted file share, and that deployment path *does* support managed identity for fetching the package. This is the actual way to eliminate the storage-account-key dependency end-to-end — but it's a plan migration, not a settings change on Consumption/Premium (see the Scope note below).
